...
const AIBraftTab: React.FC<AIBraftTabProps> = ({
  ...
  const formatMarkdownToHtml = (text: string) => {
    return text
      ...
      .replace(
        /^## (.*$)/gm,
        '<h2 class="text-xl font-semibold mb-3 mt-6 text-gray-800">$1</h2>'
      )
      .replace(
        /^### (.*$)/gm,
      ...
      .replace(/【(.*?)】/g, (match, category) => {
        ...
        return `<span class="inline-flex items-center px-2 py-1 text-xs font-medium ${
          isHighlighted
            ? "bg-blue-200 text-blue-900"
            : "bg-gray-200 text-gray-800"
        } rounded-md mr-1 cursor-pointer hover:bg-gray-300" data-category="${category}">【${category}】</span>`;
      })
  ...
  const scrollToCategory = (category: string) => {
    if (editorRef.current) {
      const categoryElement = editorRef.current.querySelector(
  ...
  const getFilteredRecords = () => {
    ...
    if (highlightedCategories.length > 0) {
      const highlighted = filtered.filter((record) =>
        highlightedCategories.includes(record.project)
      );
      const others = filtered.filter(
        (record) => !highlightedCategories.includes(record.project)
      );
  ...
  useEffect(() => {
    ...
    if (editorElement) {
      editorElement.addEventListener("click", handleCategoryClick);
      return () => {
        if (editorElement) {
          editorElement.removeEventListener("click", handleCategoryClick);
        }
      };
    }
  }, [onCategoryHighlight]);
                  ...
                  <div className="flex items-center space-x-2 mb-2">
                    <span
                      className={`inline-flex items-center px-2 py-1 text-xs font-medium rounded-md ${
                        record.project === "AI日誌報表統規劃"
                          ? "bg-blue-100 text-blue-800"
                          : record.project === "專案計畫"
                          ? "bg-purple-100 text-purple-800"
                          : record.project === "目標設定"
                          ? "bg-red-100 text-red-800"
                          : record.project === "週計畫制定"
