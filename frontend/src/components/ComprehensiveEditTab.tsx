...
const ComprehensiveEditTab: React.FC = () => {
  ...
  const startEdit = (report: ConsolidatedReport, side: 'manual' | 'ai') => {
    ...
    setEditContent(side === 'ai' ? (report.ai_content || report.content) : report.content);
  };
        ...
        <div className="w-1/2">
          <div className="bg-white rounded-lg shadow-sm border border-gray-200">
            <div className="flex items-center justify-between p-4 border-b border-gray-100">
              ...
              <button onClick={handleSubmitManual} className={`inline-flex items-center px-4 py-2 text-sm rounded-lg ${blueButtonStyle}`}>
                <Upload className="w-4 h-4 mr-2" /> 上傳手動版本
              </button>
            </div>
            <div className="p-4 space-y-4">
              {reports.map(report => (
                <div key={`manual-${report.project.id}`} className="border border-gray-200 rounded-lg p-4">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    {editingProjectId !== report.project.id && (
                      <button onClick={() => startEdit(report, 'manual')} className={`inline-flex items-center px-3 py-2 text-sm font-medium rounded-lg ${getProjectColors(report.project.name).button}`}>
                        <Edit className="w-3 h-3 mr-1" /> 編輯
                      </button>
                    )}
                  </div>

                  {editingSide === 'manual' && editingProjectId === report.project.id ? (
                    <div className="space-y-3">
                      ...
                      <div className="flex space-x-2">
                        <button onClick={saveEdit} className={`inline-flex items-center px-4 py-2 text-sm rounded-lg ${blueButtonStyle}`}>
                          <Save className="w-4 h-4 mr-2" /> 儲存
                        </button>
                        <button onClick={cancelEdit} className="px-3 py-1 bg-gray-200 text-gray-700 text-xs font-medium rounded hover:bg-gray-300">取消</button>
                      </div>
                    </div>
                  ) : (
        ...
        <div className="w-1/2">
          <div className="bg-white rounded-lg shadow-sm border border-gray-200">
            <div className="flex items-center justify-between p-4 border-b border-gray-100">
              ...
              <button onClick={handleSubmitAI} className={`inline-flex items-center px-4 py-2 text-sm rounded-lg ${blueButtonStyle}`}>
                <Upload className="w-4 h-4 mr-2" /> 上傳 AI 版本
              </button>
            </div>
            <div className="p-4 space-y-4">
              {reports.map(report => (
                <div key={`ai-${report.project.id}`} className="border border-gray-200 rounded-lg p-4">
                  <div className="flex items-center justify-between mb-3">
                    <div className={`inline-flex items-center px-3 py-1 text-sm font-medium rounded-md ${getProjectColors(report.project.name).tag}`}>
                      {report.project.name}
                    </div>
                    {report.ai_content && editingProjectId !== report.project.id && (
                      <button onClick={() => startEdit(report, 'ai')} className={`inline-flex items-center px-3 py-2 text-sm font-medium rounded-lg ${getProjectColors(report.project.name).button}`}>
                        <Edit className="w-3 h-3 mr-1" /> 編輯
                      </button>
                    )}
                  </div>
                  
                  {editingSide === 'ai' && editingProjectId === report.project.id ? (
                    <div className="space-y-3">
                      <textarea
                        value={editContent}
                        onChange={(e) => setEditContent(e.target.value)}
                    ...
                    report.ai_content ? (
                      <p className="text-sm text-gray-700 whitespace-pre-wrap">{report.ai_content}</p>
                    ) : (
                      <p className="text-sm text-gray-400 italic">請至「AI 日報編輯」分頁生成內容</p>
                    )
                  )}
                </div>
              ))}
