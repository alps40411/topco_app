...
class WorkDataService:
    ...
    def get_all_work_data(
        ...
        """
        一次性獲取填寫日報所需的所有工作資料（正規化結構）
        包含：專案列表、執行工作（去重）、專案執行工作映射、服務公司、服務對象
        """
        try:
            result = {
                "projects": [],
                "execution_works": {},  # 改為字典，以 sopno 為 key
                "project_execution_mapping": {},  # 專案與執行工作的映射
                "basic_execution_sopnos": [],  # 基本執行工作的 sopno 陣列
                "service_companies": [],
                "service_targets": []
            }
            ...
            for row in basic_exec_result:
                ...
                if current_sop != sopno:
                    if sopno not in result["execution_works"]:
                        result["execution_works"][sopno] = {
                            "sopno": sopno,
                            "sop_desc_c": sop_desc_c or f"執行工作 {sopno}",
                            "work_items": []
                        }
                        basic_sopnos.append(sopno)
                    current_sop = sopno

                # 添加工作項目
                if seq is not None and work_item_name:
                    result["execution_works"][sopno]["work_items"].append({
                        "seq": str(seq),
                        "name": work_item_name,
                        "unique_id": f"{sopno}_{seq}"
                    })

            result["basic_execution_sopnos"] = basic_sopnos
            ...
            for row in project_exec_result:
                ...
                # 初始化專案的 sopno 列表
                if planno not in result["project_execution_mapping"]:
                    result["project_execution_mapping"][planno] = []

                # 如果是新的執行工作
                if current_planno != planno or current_sop != sopno:
                    # 添加 sopno 到專案映射
                    if sopno not in result["project_execution_mapping"][planno]:
                        result["project_execution_mapping"][planno].append(sopno)
                ...
                if seq is not None and work_item_name:
                    # 檢查是否已存在相同的工作項目（避免重複）
                    existing_item = next(
                        (item for item in result["execution_works"][sopno]["work_items"]
                         if item["seq"] == str(seq)),
                        None
                    )
            ...
            logger.info(
                f"工作資料查詢成功 (正規化結構): empno={empno}, "
                f"專案={len(result['projects'])}, "
                f"執行工作總數={len(result['execution_works'])}, "
                f"基本執行工作={len(result['basic_execution_sopnos'])}, "
                f"專案執行工作映射={len(result['project_execution_mapping'])} 個專案, "
