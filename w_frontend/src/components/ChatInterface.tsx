...
const ChatInterface: React.FC<ChatInterfaceProps> = ({
  ...
  }, [initialComments, buildReplyTargets]);
  ...
  const handleSubmitMessage = async (useDefaultMessage = false) => {
    ...
    try {
      // 使用週報回覆 API，不傳評分表示一般回覆
      const forwardEmpnos = (selectedForwardUsers || [])
      ...
      const result = await WeeklyReportApi.replyWeeklyReport(
      ...
      if (result.ResponseNo === "0000") {
        ...
        throw new Error(result.ResponseNa || "提交回覆失敗");
      }
    } catch (error) {
  ...
  const handleSubmitReview = async (useDefaultComment = false) => {
    ...
    try {
      ...
      if (result.ResponseNo === "0000") {
        ...
        throw new Error(result.ResponseNa || "提交審閱失敗");
      }
    } catch (error) {
  ...
  const handleGetAISuggestions = async () => {
    ...
    try {
      const response = await authFetch(
        `/api/weekly/reports/${reportId}/ai-suggestions`,
        {
          method: "POST",
          headers: {
